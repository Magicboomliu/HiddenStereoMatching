...
from utils.image_util import resize_max_res,chw2hwc,colorize_depth_maps,resize_small_res,random_crop_batch_pil
...
class Img2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        for img in img_patches:
            ...
            # if match_input_res:
            #     rgb_pred = cv2.resize(rgb_pred, input_size, interpolation=cv2.INTER_LINEAR)
    ...
    def single_infer(self,input_rgb:torch.Tensor,
        ...
        for i, t in iterable:
            if self.use_cfg:
                ...
                zero_image = torch.zeros(input_rgb.shape, device=device, dtype=self.dtype)
