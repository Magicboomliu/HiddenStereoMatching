...
def read_annotation(annotation_filename,class_names,calib_only=False):
...
def list2txt(list_name,saved_name):
    with open(saved_name,'w') as f:
        for idx, line in enumerate(list_name):
            if idx!=len(list_name)-1:
                f.writelines(line+"\n")
            else:
                f.writelines(line)
